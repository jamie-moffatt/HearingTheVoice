...
{
	$sections = array();
	if ($result = $mysqli->query("SELECT `Section`.`sectionID` FROM `Section`, `SectionSchedule` WHERE `Section`.`sectionID` = `SectionSchedule`.`sectionID` AND `SectionSchedule`.`scheduleID` = $scheduleID ORDER BY `Section`.`sectionID`"))
	{
		while ($row = $result->fetch_assoc())
		{
